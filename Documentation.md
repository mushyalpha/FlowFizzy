...
| 12V Water Pump                     | Drives water flow from tank to nozzle |
| 5V 1-Channel Relay (Opto-isolated) | Switches the 12V pump on and off via GPIO |
| YF-S401 Flow Meter                 | Measures fill volume in ml (low flow) |
| HC-SR04 Ultrasonic Sensor + Clamp  | Detects bottle presence and monitors rising water level |
| 16x2 LCD (I2C)                    | State and fill volume display |
| Active Buzzer                      | Fill complete audio feedback |
| 1N4007 Diode                       | Flyback protection on pump relay |
...
```bash
sudo apt-get install cmake g++ libgpiod-dev qt6-base-dev libqcustomplot-dev libi2c-dev
```
...
- State machine transitions (IDLE to STABILISE to FILLING to COMPLETE)
